 ...
 * Folder: ./components/esp_iot_framework_server/src
 * File: web.c
...
#if defined(CONFIG_EIF_SERVER_ENABLE_WEB_ADMIN_GUI) || defined(CONFIG_EIF_CORE_LOG_ENABLE_REMOTE_DEBUG)
...
#ifdef CONFIG_EIF_CORE_ENABLE_TLS
    #define EIF_SERVER_TLS_VAL true
#else
    #define EIF_SERVER_TLS_VAL false
#endif
...
#define HDR_CACHE_CONTROL_VALUE "public, max-age=" EIF_STR(CONFIG_EIF_SERVER_WEB_CACHE_MAX_AGE)
...
static void httpd_resp_sendstatus(
    ...
    #ifdef CONFIG_EIF_SERVER_LOG_ENABLE_WEB_SEND_TIMESTAMP
...
static esp_err_t set_cache(httpd_req_t * const req, bool is_need) {
     ...
     */
        ...
        #ifdef CONFIG_EIF_SERVER_ENABLE_WEB_FAVICON
...
static esp_err_t h_wifi_list_json(httpd_req_t * const req) {
    ...
    /*   "used_tls": true, */
    (void)json_gen_obj_set_int(&jgen, "used_tls", EIF_SERVER_TLS_VAL);
    /*   "rssi_now_profile": -45, */
...
static esp_err_t h_sys_info_json(httpd_req_t * const req) {
    ...
    (void)set_cache(req, false);
        ...
        if (esp_clk_tree_src_get_freq_hz(
            SOC_MOD_CLK_CPU, ESP_CLK_TREE_SRC_FREQ_FOCUS_MAX, &freq_hz
        ) == ESP_OK) {
            cpu_freq = freq_hz / 1000000;
        } else {
    ...
    if (ret == ESP_OK) {
        ...
        /*   "cpu_freq": 240, */
        (void)json_gen_obj_set_int(&jgen, "cpu_freq", cpu_freq);
        /*   "reset_reason": 1, */
...
static esp_err_t h_ota_update_do(httpd_req_t * const req) {
    ...
    if (ret == ESP_OK) {
        buf = (char *)pvPortMalloc(CONFIG_EIF_SERVER_WEB_SIZE_OTA_BUFFER);
        EIF_IF_OK_CHECK_CONDITION(ret, buf == NULL, ESP_ERR_NO_MEM,
            SERVER_ERR_ALLOCATE, CONFIG_EIF_SERVER_WEB_SIZE_OTA_BUFFER, "ota_buffer");
    }
       
    while ((remaining > 0U) && (ret == ESP_OK)) {
        const bool flag = remaining < (size_t)CONFIG_EIF_SERVER_WEB_SIZE_OTA_BUFFER;
        const int chunk_size = flag ? (int)remaining : CONFIG_EIF_SERVER_WEB_SIZE_OTA_BUFFER;
...
esp_err_t eif_server_launch(void) {
    ...
    const eif_server_t * const cfg = eif_server_get();
