 ...
 * Folder: ./components/esp_iot_framework_server/include
 * File: esp_iot_framework_server.h
...
#ifndef ESP_IOT_FRAMEWORK_SERVER_H
#define ESP_IOT_FRAMEWORK_SERVER_H
...
/**
 * @defgroup server_root Node: SERVER
 * @copydoc md_docs_html_README_SERVER
 * @{
 */
/**
 ...
 * @brief SERVER REST API documentation.
 * @details @copydoc md_docs_html_REST_API_SERVER
 */ 
/**
 * @defgroup server_kconfig Kconfig
 * @brief Node SERVER `Kconfig` configuration options.
 * @copydoc md_docs_html_KCONFIG_SERVER
 */
/** @} */

/**
 * @addtogroup server_group SERVER
 * @{
 *
...
/**
 * @defgroup server_c Server Boot
 * @brief Server startup and runtime environment initialization.
 *
 * This module initializes the server hardware and prepares the runtime
...
/**
 * @brief Configures system-wide services and prepares the server networking environment.
 *
 * Sets up the base runtime environment, applies initial server configurations,
 * and binds essential system handlers required to start the server.
 *
 ...
 *
 *     #ifdef CONFIG_EIF_CORE_ENABLE_TLS
 *         ESP_ERROR_CHECK(eif_server_set_config_https(&config));
 *     #else
 *         ESP_ERROR_CHECK(eif_server_set_config_http(&config));
 *     #endif
 *
 *     // Further code...
 * }
 ...
 */
     ...
     *
     * @note This function must be called after `eif_server_initialize()` but before
     *       `eif_wifi_initialize()`.
     *
     ...
     * @endcode
     */
    esp_err_t eif_server_set_config_https(
     ...
     * @endcode
     */
    esp_err_t eif_server_set_config_http(
 ...
 *     const char *resp = "Hello from esp_iot_framework_server!";
 ...
 *     ESP_ERROR_CHECK(eif_server_initialize());
 *     ESP_ERROR_CHECK(eif_nvs_initialize());
 *     ESP_ERROR_CHECK(eif_server_set_uri_handlers(my_uris, 2));
 *
 *     // Further code...
 * }
 * @endcode
 */
esp_err_t eif_server_set_uri_handlers(
